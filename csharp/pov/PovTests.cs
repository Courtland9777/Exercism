...
public class PovTests
{
    ...
    public void Can_reroot_a_tree_with_new_root_deeply_nested_in_tree()
    {
        ...
        var from = "x";
        var expected = new Tree("x",
            new Tree("level-3", new Tree("level-2", new Tree("level-1", new Tree("level-0")))));
        Assert.Equal(expected, Pov.FromPov(tree, from));
    }
    ...
    public void Can_reroot_a_complex_tree_with_cousins()
    {
        var tree = new Tree("grandparent",
            new Tree("parent", new Tree("x", new Tree("kid-0"), new Tree("kid-1")), new Tree("sibling-0"),
                new Tree("sibling-1")), new Tree("uncle", new Tree("cousin-0"), new Tree("cousin-1")));
        var from = "x";
        var expected = new Tree("x", new Tree("kid-1"), new Tree("kid-0"),
            new Tree("parent", new Tree("sibling-0"), new Tree("sibling-1"),
                new Tree("grandparent", new Tree("uncle", new Tree("cousin-0"), new Tree("cousin-1")))));
        Assert.Equal(expected, Pov.FromPov(tree, from));
    }
    ...
    {
        var tree = new Tree("parent", new Tree("x", new Tree("kid-0"), new Tree("kid-1")), new Tree("sibling-0"),
            new Tree("sibling-1"));
        var from = "nonexistent";
        Assert.Throws<ArgumentException>(() => Pov.FromPov(tree, from));
    }
    ...
    public void Can_find_path_not_involving_root()
    {
        ...
        var to = "sibling-1";
        var tree = new Tree("grandparent",
            new Tree("parent", new Tree("x"), new Tree("sibling-0"), new Tree("sibling-1")));
