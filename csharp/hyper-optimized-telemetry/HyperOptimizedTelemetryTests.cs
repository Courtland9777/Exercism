...
public class TelemetryBufferTests
{
    ...
    public void ToBuffer_lower_neg_int()
    {
        ...
        Debug.WriteLine($"bytes array = {string.Join(',', bytes)}");
        Debug.WriteLine($"bytes array = {string.Join(',',new byte[] { 0xfc, 0x0, 0x0, 0x0, 0x80, 0x0, 0x0, 0x0, 0x0 })}");
        Debug.WriteLine($"Converted value = {BitConverter.ToInt64(new byte[] { 0xfc, 0x0, 0x0, 0x0, 0x80, 0x0, 0x0, 0x0, 0x0 }, 1)}");
        var testBytes = TelemetryBuffer.ToBuffer(Int32.MinValue);
        for (int i = 1; i < bytes.Length; i++)
        {
            testBytes[i] = (unchecked((byte)((256 + testBytes[i]) % 256)));
        }
        Debug.WriteLine($"Converted array = {string.Join(',', bytes)}  Converted value = {BitConverter.ToInt64(testBytes, 1)}");
