...
public class SgfParsingTests
{
    ...
    public void Two_nodes()
    {
        var encoded = "(;A[B];B[C])";
        var expected = new SgfTree(new Dictionary<string, string[]> { ["A"] = new[] { "B" } },
            new SgfTree(new Dictionary<string, string[]> { ["B"] = new[] { "C" } }));
        AssertEqual(expected, SgfParser.ParseTree(encoded));
    }
    ...
    public void Two_child_trees()
    {
        ...
        var expected = new SgfTree(new Dictionary<string, string[]> { ["A"] = new[] { "B" } },
            new SgfTree(new Dictionary<string, string[]> { ["B"] = new[] { "C" } }),
            new SgfTree(new Dictionary<string, string[]> { ["C"] = new[] { "D" } }));
        AssertEqual(expected, SgfParser.ParseTree(encoded));
    }
    ...
    public void Semicolon_in_property_value_doesnt_need_to_be_escaped()
    {
        ...
        var expected =
            new SgfTree(new Dictionary<string, string[]> { ["A"] = new[] { "a;b", "foo" }, ["B"] = new[] { "bar" } },
                new SgfTree(new Dictionary<string, string[]> { ["C"] = new[] { "baz" } }));
        AssertEqual(expected, SgfParser.ParseTree(encoded));
    }
    ...
    public void Parentheses_in_property_value_dont_need_to_be_escaped()
    {
        ...
        var expected =
            new SgfTree(new Dictionary<string, string[]> { ["A"] = new[] { "x(y)z", "foo" }, ["B"] = new[] { "bar" } },
                new SgfTree(new Dictionary<string, string[]> { ["C"] = new[] { "baz" } }));
        AssertEqual(expected, SgfParser.ParseTree(encoded));
    }
    ...
    private void AssertEqual(SgfTree expected, SgfTree actual)
    {
        ...
        for (var i = 0; i < expected.Children.Length; i++) AssertEqual(expected.Children[i], actual.Children[i]);
    }
}